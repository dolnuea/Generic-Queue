/**
 The test1 class uses an array to implement a queue.
 */
...
class test1
{
    ...
    /**
     ...
     @exception DD_QueueOverFlowException When there
    ...
    {
        if (size == q.length)
            throw new DD_QueueOverFlowException();
        else
        {
            // Add to rear
            size ++;
    ...
    {
        if (empty())
            throw new DD_EmptyQueueException();
        else
            return q[front];
    }
